...
test("keeps the local bridge narrow and Claude Code-specific", async () => {
  ...
  assert.match(bridge, /BRIDGE_PROTOCOL = 10/);
  assert.match(launcher, /bridgeProtocol -ge 10/);
  ...
  assert.match(bridge, /\/api\/providers\/deepseek/);
  assert.match(bridge, /deepseek-provider/);
  assert.match(bridge, /secure-provider-store/);
  assert.match(bridge, /deepseek-v4-pro\[1m\]/);
  assert.match(bridge, /DeepSeek API Key 无效或已失效/);
  assert.match(bridge, /sk-\[\^/);
  assert.match(bridge, /isLocalOrigin/);
  assert.match(bridge, /STATE_METHODS/);
  assert.match(bridge, /deepSeekChildEnvironment/);
  assert.match(bridge, /CLAUDE_CODE_SUBAGENT_MODEL/);
  assert.match(bridge, /ANTHROPIC_BASE_URL = DEEPSEEK_BASE_URL/);
  assert.match(bridge, /protectedData.+dataProtectionScope|ProtectedData/);
  assert.match(bridge, /windowsHide: true/);
  assert.match(page, /DeepSeek V4|连接 DeepSeek API/);
  assert.match(page, /deepseek-mark/);
  assert.match(page, /DEEPSEEK_MODEL_OPTIONS/);
  assert.match(page, /mapping-note/);
  ...
  assert.match(page, /sk-…/);
  assert.match(page, /当前本地桥接不支持 DeepSeek/);
  assert.match(page, /DEEPSEEK_SOURCE_LABEL/);
  assert.match(page, /sessionModelUsage/);
  assert.doesNotMatch(page, /localStorage\.setItem\([^)]*deepseek/i);
